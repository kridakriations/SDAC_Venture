...
onready var grenade_spawn_point = $"%grenade_spawn_point"
...
func swap():
	...
	if(Server.network.get_unique_id() == 1):
		Server.swap_gun_over_network(1)
	else:
		Server.rpc_unreliable_id(1,"swap_gun_over_network",Server.network.get_unique_id())
...
#		print("grenade")
		...
		$ColorRect/grenade_count.text = str(bomb)
		if(Server.network.get_unique_id() == 1):
			Server.bomb_thrown_over_network(1,input["grenade"])
		else:
			Server.rpc_id(1,"bomb_thrown_over_network",Server.network.get_unique_id(),input["grenade"])
		
	if (input["shoot"]["is_shot"] == false):
		gun_node.get_child(0).curr_spread = 0
		gun_node.get_child(1).curr_spread = 0
				
	if ((input["shoot"]["is_shot"] == true) and (busy_frames <= 0) and (died == false)):
		if (gun_node.get_child(gun_no).can_shoot() and busy_frames <= 0):
			...
			if(Server.network.get_unique_id() == 1):
				Server.enemy_gun_shot_over_network(1,input["shoot"]["target"])
			else:
				Server.rpc_unreliable_id(1,"enemy_gun_shot_over_network",Server.network.get_unique_id(),input["shoot"]["target"])
			...
			if(is_instance_valid(Server.world_node)):
				...
				if(col_dict.empty() == false):
					...
					var victim_id 
					var shot_type
					if(col_dict.collider.is_in_group("enemy_head")):
						damage = Gamedata.gun_stats[gun_node.get_child(gun_no).gun_type].head_damage
						victim_id = col_dict.collider.get_parent().get_parent().id
						shot_type = "HEADSHOT"
					elif(col_dict.collider.is_in_group("enemy_body")):
						damage = Gamedata.gun_stats[gun_node.get_child(gun_no).gun_type].damage
						victim_id = col_dict.collider.get_parent().id
						shot_type = "BODYSHOT"
					
					if(damage != 0):
						add_damage_indicator(damage)
						if(Server.network.get_unique_id() == 1):
							Server.player_hit(1,victim_id,damage,shot_type,global_transform.origin)
						else:
							Server.rpc_id(1,"player_hit",Server.network.get_unique_id(),victim_id,damage,shot_type,global_transform.origin)
						pass
...
func _physics_process(delta):
	if(sense == true):
		...
		if Input.is_action_just_pressed("grenade"):
			...
			input_packet["grenade"]["source"] = grenade_spawn_point.global_transform.origin#head.global_transform.origin
	
	
	var sim_state = process_input(input_packet,false)#proccesing the input
	var new_player_state = {}
	new_player_state.player_position = global_transform.origin
	new_player_state.head_rotation = get_node("body").get_node("head").rotation_degrees.x
	new_player_state.body_rotation = get_node("body").rotation_degrees.y
	new_player_state.left_leg_rotation = left_leg_rotation
	new_player_state.right_leg_rotation = right_leg_rotation
	new_player_state.step_sound = $steps.playing
#	if(send_update):
#	print("connection status ",Server.network.get_connection_status())
	if(Server.network.get_connection_status() == 2):
		if(Server.network.get_unique_id() == 1):
			Server.update_player_state(new_player_state,true)
		else:
			Server.rpc_id(1,"update_player_state",new_player_state)
	clear_input()
