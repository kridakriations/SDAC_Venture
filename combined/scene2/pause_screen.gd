...
func set_settings():
	...
	print(settings)
	if((settings == null) or (settings.empty() == false)):
...
func _on_pause_screen_visibility_changed():
	if(visible == true or get_parent().get_node("game_end_screen").visible == true):
		set_settings()
...
#	var is_mute = AudioServer.is_bus_mute(AudioServer.get_bus_index("sfx"))
	...
	if(new_settings["shading"] == true):
		...
		if(get_parent().map_id == 1):
			get_parent().get_node("level").get_child(0).get_node("BakedLightmap").light_data = load("res://new_light_map_dio/dio_map_scene_test2.lmbake")
	else:
